...
$.autocomplete = function(input, options) {
	...
	function onChange() {
		...
		var v = $input.val();
		if(options.chk){if(!options.chk(v)) return;};
		if (v == prev) return;
		prev = v;
	...
	function selectItem(li) {
		...
		prev = v;
		$input.val(v);
		//IE下有问题:$results.html("");原位置
		hideResultsNow();
		if (options.onItemSelect) options.onItemSelect(li);
		//IE下有问题:$results.html("");改放在最后,将上句改为直接调用，原句要setTimeOut
		$results.html("");
	};
	...
	function makeUrl(q) {
		...
		var ex = ($.type(options.extraParams)=="function") ? options.extraParams() :options.extraParams;
		for (var i in ex) {
			url += "&" + i + "=" + encodeURI(ex[i]);
		}
		return url;
	};
